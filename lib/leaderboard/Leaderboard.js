...
export default class Leaderboard {
  ...
  query(filter = {}) {
    return this.cache.filter(entry => !(
      ...
      || filter.strict ? filter.constellations !== entry.noelle.constellations : filter.constellations < entry.noelle.constellations
      || filter.strict ? filter.refinement !== (entry.noelle.weapon.refinement + 1) : filter.refinement < (entry.noelle.weapon.refinement + 1)
