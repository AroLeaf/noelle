...
export default new PrefixCommand({
  name: 'build',
  description: `
    Shows the build of a user. This will be their up-to-date build, and might not match the build they submitted to the leaderboard.
    ...
    If no user is provided, it defaults to the user who issued the command.
  `,
  args: [{
  ...
  return message.reply(Object.assign(DME.render(`
    ...
    - Main Stats: ${noelle.artifacts.filter(a => !['HP', 'ATK'].includes(a.mainStat)).map(a => `**${constants.PROPER_PROP_NAMES[a.mainStat]}${a.mainStat.startsWith('P') ? '%' : ''}**`).join('/')}
